...
// Upsert por docId determinístico (wc_{id})
async function firestoreSet(docId, fields) {
  await axios.patch(`${BASE_URL}/produtos/${docId}?key=${FIREBASE_API_KEY}`, { fields });
}

async function firestoreGetAll() {
  const res = await axios.get(`${BASE_URL}/produtos?pageSize=200&key=${FIREBASE_API_KEY}`);
  return (res.data.documents || []).map(d => {
    const f = d.fields || {};
    const id = d.name.split('/').pop();
    const get = (k, fallback = '') => f[k]?.stringValue ?? f[k]?.doubleValue ?? f[k]?.booleanValue ?? fallback;
    return {
      id, wcId: get('wcId'), nome: get('nome'), sku: get('sku'),
      preco: get('preco', 0), precoRegular: get('precoRegular', 0),
      estoque: get('estoque', 0), status: get('status'), categoria: get('categoria'),
      imagem: get('imagem'), descricao: get('descricao'), permalink: get('permalink'),
...
export default async function handler(req, res) {
  ...
  // GET /api/produtos          → retorna cache Firestore
  // GET /api/produtos?sync=1   → sincroniza WooCommerce primeiro
  if (req.method === 'GET') {
    try {
      if (req.query.sync === '1') {
  ...
  // POST /api/produtos  → força sync completo
  if (req.method === 'POST') {
    try {
      const total = await sincronizarWooCommerce();
