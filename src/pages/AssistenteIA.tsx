    ...
    <div className="flex h-screen bg-slate-950 text-slate-100">
      ...
      <div className="w-1/2 border-r border-slate-800 flex flex-col">
        ...
        <div className="flex-1 p-4 space-y-4 overflow-y-auto">
          {messages.map((m, i) => <div key={i} className="bg-slate-900 p-3 rounded-lg">{m}</div>)}
          {isAnalyzing && <Loader2 className="animate-spin text-blue-500" />}
        </div>
        <div className="p-4 border-t border-slate-800 space-y-2">
          <button onClick={simulateOCR} className="w-full bg-slate-800 p-4 rounded-xl border-2 border-dashed border-slate-600 hover:border-blue-500 flex flex-col items-center gap-2">
