    ...
    <div className="h-full flex flex-col gap-6 p-2">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        <div className="bg-slate-900 border border-slate-800 p-6 rounded-xl">
          <p className="text-slate-400 text-sm flex items-center gap-2"><Building2 size={16} /> Total Investido</p>
          <p className="text-3xl font-bold text-white">R$ {kpis.totalInvestido.toLocaleString('pt-BR', { minimumFractionDigits: 2 })}</p>
        </div>
        <div className="bg-slate-900 border border-slate-800 p-6 rounded-xl">
          <p className="text-slate-400 text-sm flex items-center gap-2"><Clock size={16} className="text-blue-500" /> Valor Patrimonial Atual</p>
          <p className="text-3xl font-bold text-blue-400">R$ {kpis.valorAtual.toLocaleString('pt-BR', { minimumFractionDigits: 2 })}</p>
        </div>
        <div className="bg-slate-900 border border-slate-800 p-6 rounded-xl">
          <p className="text-slate-400 text-sm flex items-center gap-2"><AlertTriangle size={16} className="text-amber-500" /> Equipamentos Parados/Manutenção</p>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        {mockEquipamentos.map((eq) => {
          ...
          return (
            <div key={eq.id} className="bg-slate-900 border border-slate-800 p-6 rounded-xl flex flex-col gap-4">
              ...
              <h3 className="text-lg font-bold text-white">{eq.nome}</h3>
              ...
              <div className="mt-auto pt-4 border-t border-slate-800 text-sm text-slate-300">
                <p>Próxima Manutenção: <span className="font-bold text-white">{eq.proximaManutencao}</span></p>
              </div>
            </div>
          );
        })}
