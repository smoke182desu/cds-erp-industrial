...
export const EntradaInteligente: React.FC = () => {
  ...
  const handleGerarPO = () => {
    ...
    alert(`Pedido de Compra Gerado:\n\n${poContent}\n\n"Solicitamos cotação/faturamento dos itens acima para entrega na CDS Industrial. Faturamento para 30/60 dias."`);
  };
    ...
    <div className="p-6 bg-slate-900 min-h-screen text-slate-100">
      ...
      <div className="flex gap-4 mb-6">
        <button 
          ...
          className={`px-4 py-2 rounded-lg ${activeTab === 'necessidade' ? 'bg-blue-600' : 'bg-slate-800'}`}
        >
          🚨 Necessidade de Compra
        </button>
        <button 
          onClick={() => setActiveTab('entrada')}
          className={`px-4 py-2 rounded-lg ${activeTab === 'entrada' ? 'bg-blue-600' : 'bg-slate-800'}`}
        >
      ...
      {activeTab === 'entrada' && (
        <div className="bg-slate-800 p-6 rounded-xl max-w-md">
