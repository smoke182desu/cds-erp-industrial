...
const KPI = ({ title, value, icon: Icon, color }: { title: string; value: string; icon: any; color: string }) => (
  <div className="bg-slate-900 p-6 rounded-2xl border border-slate-800 flex items-center gap-4 shadow-lg shadow-black/20">
...
export const DashboardBI: React.FC = () => {
  ...
  const kpis = useMemo(() => {
    ...
    const alertasEstoque = state.inventoryItems.filter(i => i.quantidadeEstoque <= i.estoqueMinimo).length;
  ...
  const atividades = useMemo(() => {
    return [
      ...state.propostas.map(p => ({ desc: `Nova Proposta: ${p.clienteNome}`, valor: p.total, date: p.data })),
  ...
  return (
    <div className="p-6 bg-slate-950 min-h-screen text-slate-100 space-y-6">
      <h2 className="text-3xl font-bold text-white mb-8">Cockpit Executivo</h2>
