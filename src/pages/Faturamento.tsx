  ...
  return (
    <div className="h-full flex flex-col gap-6 p-2">
      <div className="flex justify-between items-center">
        ...
        <div className="bg-white p-2 rounded-lg border border-slate-300 flex items-center gap-4">
        ...
        <table className="w-full text-left text-sm text-slate-700">
          ...
          <tbody>
            {state.budget.items.map((projeto) => (
              <tr key={projeto.id} className="border-b border-slate-200 hover:bg-slate-100/50">
                <td className="px-6 py-4 font-medium text-slate-900">{projeto.nome}</td>
                <td className="px-6 py-4">{projeto.cliente || 'Cliente Padrão'}</td>
                <td className="px-6 py-4">
                  <span className={`px-2 py-1 rounded text-xs font-bold ${
                    status[projeto.id] === 'Autorizado' ? 'bg-emerald-100 text-emerald-700' :
                    status[projeto.id] === 'Erro' ? 'bg-rose-100 text-rose-700' : 'bg-slate-100 text-slate-600'
                  }`}>
