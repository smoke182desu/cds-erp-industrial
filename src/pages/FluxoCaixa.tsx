    ...
    <div className="p-6 bg-slate-50 min-h-screen text-slate-900">
      ...
      <div className="bg-white p-6 rounded-2xl border border-slate-200">
        ...
        <table className="w-full text-sm">
          ...
          <tbody>
            {transacoes.map(t => (
              <tr key={t.id} className="border-b border-slate-200 hover:bg-slate-100/50">
                ...
                <td className="p-3 text-center">
                  <span className={`px-2 py-1 rounded text-xs ${t.status === 'PAGO' ? 'bg-green-100 text-green-700' : 'bg-yellow-100 text-yellow-700'}`}>
                    {t.status}
                  </span>
                </td>
                <td className="p-3 text-center">
