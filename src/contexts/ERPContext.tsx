...
import { Cliente, Proposta, OrdemServico, Receita, Faturamento, Component, TransacaoFinanceira } from '../types';
...
interface ERPContextType {
  ...
  fecharProposta: () => void;
  aprovarVenda: (clienteId: string) => void;
  salvarRascunho: (clienteId: string) => void;
...
export const ERPProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  const [state, setState] = useState<ERPState>(() => {
    ...
    const inventoryItems = savedInventory ? JSON.parse(savedInventory) : estoqueInicial;
    const clientes = savedClientes ? JSON.parse(savedClientes) : [
      { id: '1', nome: 'Construtora Exemplo', email: 'contato@exemplo.com', telefone: '11999999999', tipo: 'PJ', documento: '12.345.678/0001-90', endereco: 'Rua A, 100' },
      { id: '2', nome: 'Metalúrgica Teste', email: 'vendas@teste.com', telefone: '11888888888', tipo: 'PJ', documento: '98.765.432/0001-10', endereco: 'Rua B, 200' }
    ];
  ...
  const validarConciliacao = (pedidoId: string, itensConciliados: { itemId: string; qtd: number; preco: number }[]) => {
    setState(prev => {
      ...
      const novasReceitas = [...prev.financeiro];
  ...
  const aprovarVenda = (clienteId: string) => {
    ...
    try {
      ...
      setState(prev => {
        ...
        (prev.carrinhoAtual || []).forEach(item => {
          // Se o item não existe no estoque, inicializa com 0
          const itemId = item?.id || 'item-generico';
          if (novoEstoque[itemId] === undefined) {
            novoEstoque[itemId] = 0;
          }
          
          novoEstoque[itemId] = novoEstoque[itemId] - 1;

          // Gatilho de Auto-Compra para Estoque Negativo
          if (novoEstoque[itemId] < 0) {
            ...
            novosPedidosCompra.push({
              ...
              itens: [{ 
                id: itemId, 
                nome: item?.nome || item?.name || 'Item Faltante', 
                qtd: Math.abs(novoEstoque[itemId]), 
                preco: (item?.custos?.material || 100) * 0.9 // Preço de custo estimado
              }],
  ...
  const salvarRascunho = (clienteId: string) => {
    if (!state.carrinhoAtual || state.carrinhoAtual.length === 0) {
  ...
  const moverEtapaOS = (osId: string, novaEtapa: OrdemServico['status']) => {
    setState(prev => {
      ...
      // Se mover para 'Corte e Dobra', desconta estoque
      if (novaEtapa === 'Corte e Dobra' && os && os.status === 'Fila de Produção') {
        os.itens.forEach(item => {
          const itemId = item.id || 'item-generico';
          novoEstoque[itemId] = (novoEstoque[itemId] || 0) - 1;
  ...
  return (
    <ERPContext.Provider value={{ state, adicionarAoCarrinho, removerDoCarrinho, fecharProposta, aprovarVenda, salvarRascunho, gerarOS, moverEtapaOS, updateInventory, calculateBOM, moverEtapaProjeto, totalCarrinho, validarConciliacao, limparCarrinho, adicionarCliente, darEntradaEstoque, registrarTransacao, atualizarStatusTransacao }}>
      {children}
    </ERPContext.Provider>
  );
};
