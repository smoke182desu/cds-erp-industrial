...
const sections: SidebarSection[] = [
  {
    ...
    items: [
      ...
      { name: 'Leads',            icon: MessageSquare,   id: 'leads' },
    ...
    .map(section => ({
      ...
      items: section.items.filter(item => {
        ...
        if (user.role === 'VENDEDOR') {
          return [
            'dashboard', 'projetos-3d', 'crm', 'assistente-ia',
            'propostas', 'checkout', 'licitacoes', 'locacoes', 'leads',
          ].includes(item.id);
        }
        if (user.role === 'OPERADOR') {
          return [
            'corte', 'dobra', 'solda', 'pintura', 'embalagem',
            'entrega', 'pcp-kanban', 'faturamento', 'estoque', 'entrada-inteligente',
          ].includes(item.id);
        }
        return false;
      }),
                ...
                {section.items.map((item) => (
                  <button
                    ...
                    className={`w-full flex items-center gap-3 px-6 py-2.5 text-sm transition-all border-l-2 ${
                      activeTab === item.id
                        ? 'bg-white border-blue-500 text-slate-900 font-medium'
