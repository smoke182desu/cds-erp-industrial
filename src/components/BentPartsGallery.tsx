...
export function BentPartsGallery({ project }: BentPartsGalleryProps) {
  ...
  return (
    <div className="p-8 max-w-7xl mx-auto relative min-h-screen">
      <div className="flex flex-col md:flex-row items-start md:items-center justify-between mb-8 gap-4">
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-sm text-slate-500">Detalhamento técnico para fabricação</p>
          </div>
        </div>
        
        <button 
          onClick={() => setIsMaterialListOpen(true)}
          className="flex items-center gap-2 px-4 py-2 bg-white border border-slate-200 text-slate-700 font-bold rounded-lg hover:bg-slate-50 transition-colors shadow-sm"
        >
        ...
        <div className="bg-white p-4 rounded-xl border border-slate-200 shadow-sm flex items-center gap-6">
          <div className="flex flex-col">
            <span className="text-xs font-bold text-slate-400 uppercase tracking-wider">Tempo Total de Produção</span>
            <div className="flex items-baseline gap-1">
              <span className="text-2xl font-bold text-slate-800">{Math.ceil(totalProjectTime / 60)}h {totalProjectTime % 60}m</span>
              <span className="text-xs text-slate-500">({totalProjectTime} min)</span>
            </div>
          </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        {parts.map((part) => (
          <div 
            ...
            className="bg-white rounded-xl border border-slate-200 shadow-sm hover:shadow-md hover:border-blue-400 transition-all cursor-pointer overflow-hidden flex flex-col group"
          >
            <div className={`h-32 ${part.color} bg-opacity-30 flex items-center justify-center relative border-b border-slate-100 group-hover:bg-opacity-40 transition-colors`}>
              ...
              <div className="opacity-60 transform group-hover:scale-105 transition-transform duration-300">
                 {part.type === 'U-Profile' && <div className="w-24 h-12 border-b-4 border-x-4 border-slate-700 rounded-b-sm"></div>}
                 {part.type === 'L-Shape' && <div className="w-24 h-16 border-b-4 border-l-4 border-slate-700 rounded-bl-sm"></div>}
                 {part.type === 'Flat' && <div className="w-24 h-16 border-4 border-slate-700 bg-slate-300/20"></div>}
                 {part.type === 'Trapezoid' && <div className="w-24 h-16 border-4 border-slate-700 transform -skew-x-12"></div>}
                 {part.type === 'Bent' && <div className="w-24 h-16 border-4 border-slate-700 rounded-sm"></div>}
              </div>
              
              <span className="absolute bottom-2 right-2 text-[10px] font-mono font-bold bg-white/80 px-1.5 py-0.5 rounded text-slate-600 border border-slate-200">
                {part.type}
              </span>
            </div>
              ...
              <div className="space-y-2 mt-auto">
                <div className="flex justify-between items-center text-sm border-b border-slate-100 pb-2">
                  ...
                  <div className="flex flex-col items-end">
                    ...
                    <span className="text-xs text-slate-500 flex items-center gap-1 font-medium text-amber-600 bg-amber-50 px-1.5 py-0.5 rounded mt-1">
      ...
      <div id="welding-project" className="mt-12 bg-white p-8 rounded-2xl border border-slate-200 shadow-sm text-center">
...
function BlueprintView({ part, isUnfolded }: { part: PartData, isUnfolded: boolean }) {
  ...
  return (
    <div className="w-full h-full flex flex-col items-center justify-center bg-white p-8">
      <div className="w-full max-w-4xl border-2 border-slate-200 rounded-lg overflow-hidden shadow-inner bg-slate-50 relative">
        ...
        <div className="absolute top-4 left-4 bg-white/80 backdrop-blur-sm p-3 rounded border border-slate-200 shadow-sm text-[10px] font-mono uppercase tracking-wider text-slate-500">
        ...
        <div className="bg-slate-50 border-b border-slate-200 p-4 flex justify-between items-center z-10">
          <div className="flex items-center gap-3">
             ...
             <div>
               ...
               <p className="text-sm text-slate-500 font-mono">{part.id} - {part.type} - Espessura: {part.thickness}mm</p>
             </div>
          </div>
          <div className="flex items-center gap-2">
            <div className="flex bg-slate-100 p-1 rounded-xl mr-4">
              <button 
                ...
                className={`px-3 py-1.5 rounded-lg text-sm font-bold transition-all flex items-center gap-2 ${viewType === '3d' ? 'bg-white text-blue-600 shadow-sm' : 'text-slate-500 hover:text-slate-700'}`}
              >
                <Rotate3d size={16} /> 3D
              </button>
              <button 
                onClick={() => setViewType('blueprint')}
                className={`px-3 py-1.5 rounded-lg text-sm font-bold transition-all flex items-center gap-2 ${viewType === 'blueprint' ? 'bg-white text-blue-600 shadow-sm' : 'text-slate-500 hover:text-slate-700'}`}
              >
            ...
            <div className="flex bg-slate-100 p-1 rounded-xl mr-4">
              <button 
                ...
                className={`px-4 py-1.5 rounded-lg text-sm font-bold transition-all flex items-center gap-2 ${viewMode === 'specs' ? 'bg-white text-blue-600 shadow-sm' : 'text-slate-500 hover:text-slate-700'}`}
              >
                <Calculator size={16} /> Especificações
              </button>
              <button 
                onClick={() => setViewMode('welding')}
                className={`px-4 py-1.5 rounded-lg text-sm font-bold transition-all flex items-center gap-2 ${viewMode === 'welding' ? 'bg-white text-red-600 shadow-sm' : 'text-slate-500 hover:text-slate-700'}`}
              >
        ...
        <div className="flex-1 flex flex-col lg:flex-row overflow-hidden">
          ...
          <div className="flex-1 bg-slate-100 relative overflow-hidden group">
             ...
             <div className="absolute top-4 right-4 flex flex-col gap-2 z-10 opacity-0 group-hover:opacity-100 transition-opacity duration-300">
                {viewMode === 'welding' && (
                  <div className="bg-white/90 backdrop-blur p-2 rounded-lg shadow-sm border border-slate-200 flex flex-col gap-2 mb-2">
                    <div className="flex items-center gap-2 border-b border-slate-100 pb-2 mb-1">
                       <span className="text-xs font-bold text-slate-500 uppercase">Tipo de Solda</span>
                    </div>
                ...
                <div className="bg-white/90 backdrop-blur p-2 rounded-lg shadow-sm border border-slate-200 flex flex-col gap-2">
                   <div className="flex items-center gap-2 border-b border-slate-100 pb-2 mb-1">
                ...
                <div className="bg-white/90 backdrop-blur p-2 rounded-lg shadow-sm border border-slate-200 w-48">
                   ...
                   {showDimensions && (
                     <div className="space-y-3">
                       <div className="bg-slate-50 p-2 rounded border border-slate-100">
                          <span className="text-[10px] font-bold text-slate-400 uppercase block mb-1">Tipo de Medida</span>
                          <div className="flex gap-1">
                             <button 
                               onClick={() => setDimensionType('external')}
                               className={`flex-1 py-1 text-[10px] font-bold rounded border ${dimensionType === 'external' ? 'bg-white border-blue-300 text-blue-600 shadow-sm' : 'border-transparent text-slate-500 hover:bg-slate-100'}`}
                             >
                               Externa
                             </button>
                             <button 
                               onClick={() => setDimensionType('internal')}
                               className={`flex-1 py-1 text-[10px] font-bold rounded border ${dimensionType === 'internal' ? 'bg-white border-blue-300 text-blue-600 shadow-sm' : 'border-transparent text-slate-500 hover:bg-slate-100'}`}
                             >
                               Interna
                             </button>
                             <button 
                               onClick={() => setDimensionType('both')}
                               className={`flex-1 py-1 text-[10px] font-bold rounded border ${dimensionType === 'both' ? 'bg-white border-blue-300 text-blue-600 shadow-sm' : 'border-transparent text-slate-500 hover:bg-slate-100'}`}
                             >
                               Ambas
                             </button>
                          </div>
                       ...
                       <label className="flex items-center gap-2 text-xs text-slate-700 cursor-pointer hover:bg-slate-50 p-1 rounded">
                         <input type="checkbox" checked={dimensionConfig.width} onChange={e => setDimensionConfig({...dimensionConfig, width: e.target.checked})} className="rounded border-slate-300 text-blue-600 focus:ring-blue-500" />
                         Largura (W)
                       </label>
                       <label className="flex items-center gap-2 text-xs text-slate-700 cursor-pointer hover:bg-slate-50 p-1 rounded">
                         <input type="checkbox" checked={dimensionConfig.height} onChange={e => setDimensionConfig({...dimensionConfig, height: e.target.checked})} className="rounded border-slate-300 text-blue-600 focus:ring-blue-500" />
                         Altura (H)
                       </label>
                       {part.dimD && (
                          <label className="flex items-center gap-2 text-xs text-slate-700 cursor-pointer hover:bg-slate-50 p-1 rounded">
                            <input type="checkbox" checked={dimensionConfig.depth} onChange={e => setDimensionConfig({...dimensionConfig, depth: e.target.checked})} className="rounded border-slate-300 text-blue-600 focus:ring-blue-500" />
                            Aba / Prof. (D)
                          </label>
                       )}
                       <label className="flex items-center gap-2 text-xs text-slate-700 cursor-pointer hover:bg-slate-50 p-1 rounded">
                         <input type="checkbox" checked={dimensionConfig.thickness} onChange={e => setDimensionConfig({...dimensionConfig, thickness: e.target.checked})} className="rounded border-slate-300 text-blue-600 focus:ring-blue-500" />
                         Espessura
                       </label>
                     </div>
                   </div>
             ...
             {viewMode === 'specs' ? (
                    ...
                    <div className="space-y-6">
                       <div className="bg-white p-5 rounded-xl border border-slate-200 shadow-sm">
                         ...
                         <div className="grid grid-cols-2 gap-y-4 gap-x-2 mt-3">
                            <div>
                              <div className="text-xs text-slate-500 mb-1">Largura (W)</div>
                              <div className="font-mono font-bold text-xl text-slate-800">{part.dimW} <span className="text-xs text-slate-400">mm</span></div>
                            </div>
                            <div>
                              <div className="text-xs text-slate-500 mb-1">Altura (H)</div>
                              <div className="font-mono font-bold text-xl text-slate-800">{part.dimH} <span className="text-xs text-slate-400">mm</span></div>
                            </div>
                            {part.dimD && (
                              <div className="col-span-2 bg-slate-50 p-2 rounded border border-slate-100">
                                <div className="text-xs text-slate-500 mb-1">Aba / Profundidade (D)</div>
                                <div className="font-mono font-bold text-xl text-slate-800">{part.dimD} <span className="text-xs text-slate-400">mm</span></div>
                              </div>
                            )}
                         </div>
                       </div>
                           ...
                           <div className="space-y-2 text-sm">
                             <div className="flex justify-between border-b border-amber-100 pb-1">
                               ...
                               <div className="text-right">
                                 <span className="text-xs text-slate-400 line-through mr-2">Ext: {part.dimW}mm</span>
                             ...
                             <div className="flex justify-between border-b border-amber-100 pb-1">
                               ...
                               <div className="text-right">
                                 <span className="text-xs text-slate-400 line-through mr-2">Ext: {part.dimH}mm</span>
                             ...
                             {part.dimD && (
                               <div className="flex justify-between border-b border-amber-100 pb-1">
                                 ...
                                 <div className="text-right">
                                   <span className="text-xs text-slate-400 line-through mr-2">Ext: {part.dimD}mm</span>
               ...
               <div className="p-6 flex flex-col h-full bg-slate-50">
                  ...
                  <div className="bg-white p-4 rounded-xl border border-slate-200 shadow-sm mb-6">
                    ...
                    <div className="space-y-3">
                      <div className="flex items-center justify-between p-2 bg-slate-50 rounded border border-slate-100">
                        <span className="text-xs font-bold text-slate-500">Arestas Selecionadas</span>
                      ...
                      <div className="flex items-center justify-between p-2 bg-slate-50 rounded border border-slate-100">
                        <span className="text-xs font-bold text-slate-500">Interseções</span>
                  ...
                  <div className="flex-1 overflow-y-auto space-y-4">
                    <div>
                      <h4 className="text-[10px] font-bold text-slate-400 uppercase tracking-widest mb-2">Soldas</h4>
                      {activeWelds.length === 0 ? (
                        <div className="text-center py-4 border border-dashed border-slate-200 rounded-lg">
                          <p className="text-[10px] text-slate-400">Nenhuma solda.</p>
                        </div>
                      ) : (
                        <div className="space-y-1">
                          {activeWelds.map(w => (
                            <div key={w} className="flex items-center justify-between p-2 bg-white rounded border border-slate-200 shadow-sm">
                              <span className="text-[10px] font-bold text-slate-700">Aresta {w.split('-')[1]}</span>
                              <button onClick={() => toggleWeld(w)} className="text-slate-300 hover:text-red-500"><X size={12} /></button>
                            </div>
                          ))}
                        </div>
                      )}
                    ...
                    <div>
                      <h4 className="text-[10px] font-bold text-slate-400 uppercase tracking-widest mb-2">Interseções</h4>
                      {activeIntersections.length === 0 ? (
                        <div className="text-center py-4 border border-dashed border-slate-200 rounded-lg">
                          <p className="text-[10px] text-slate-400">Nenhuma interseção.</p>
                        </div>
                      ) : (
                        <div className="space-y-1">
                          {activeIntersections.map(i => (
                            <div key={i} className="flex items-center justify-between p-2 bg-white rounded border border-slate-200 shadow-sm">
                              ...
                              <button onClick={() => toggleIntersection(i)} className="text-slate-300 hover:text-red-500"><X size={12} /></button>
                            </div>
                          ))}
                        </div>
                      )}
             ...
             {showIssueReport && (
               <div className="absolute inset-0 bg-white/95 backdrop-blur z-30 p-6 flex flex-col animate-in fade-in slide-in-from-bottom-4 duration-200">
                 ...
                 <div className="space-y-4 flex-1">
                   ...
                   <div>
                     <label className="block text-xs font-bold text-slate-500 uppercase mb-1">O que aconteceu?</label>
                   ...
                   <div>
                     ...
                     <textarea 
                       className="w-full p-3 border border-slate-300 rounded-lg text-sm h-32 resize-none focus:ring-2 focus:ring-amber-500 focus:border-amber-500 outline-none"
                       placeholder="Ex: A chapa foi cortada com 1180mm ao invés de 1200mm..."
                     ></textarea>
                   </div>
                 </div>

                 <button className="w-full py-3 bg-slate-900 text-white rounded-lg font-bold hover:bg-slate-800 transition-colors shadow-lg">
                   Enviar Reporte
                 </button>
               </div>
             )}
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-900/90 backdrop-blur-md p-4">
      <div className="bg-white w-full max-w-7xl h-[90vh] rounded-3xl shadow-2xl flex flex-col overflow-hidden relative border border-white/20">
        ...
        {/* Header */}
        <div className="bg-slate-50 border-b border-slate-200 p-6 flex justify-between items-center z-10">
          <div className="flex items-center gap-4">
             ...
             <div>
               ...
               <p className="text-sm text-slate-500">Defina cordões, costuras e interseções na montagem final</p>
             </div>
          </div>
          
          <div className="flex items-center gap-4">
            <label className="flex items-center gap-2 bg-white px-3 py-2 rounded-lg border border-slate-200 shadow-sm cursor-pointer hover:bg-slate-50 transition-colors">
              <input 
                type="checkbox" 
                checked={showMDF} 
            ...
            <div className="flex bg-slate-200 p-1 rounded-xl border border-slate-300">
              <button 
                onClick={() => setWeldMode('weld')}
                className={`px-4 py-2 rounded-lg text-xs font-bold transition-all ${weldMode === 'weld' ? 'bg-white text-blue-600 shadow-md' : 'text-slate-500 hover:text-slate-700'}`}
              >
                Contínua
              </button>
              <button 
                onClick={() => setWeldMode('stitch-short')}
                className={`px-4 py-2 rounded-lg text-xs font-bold transition-all ${weldMode === 'stitch-short' ? 'bg-white text-indigo-600 shadow-md' : 'text-slate-500 hover:text-slate-700'}`}
              >
                F. Curta
              </button>
              <button 
                onClick={() => setWeldMode('stitch-long')}
                className={`px-4 py-2 rounded-lg text-xs font-bold transition-all ${weldMode === 'stitch-long' ? 'bg-white text-indigo-600 shadow-md' : 'text-slate-500 hover:text-slate-700'}`}
              >
                F. Longa
              </button>
              <button 
                onClick={() => setWeldMode('spot')}
                className={`px-4 py-2 rounded-lg text-xs font-bold transition-all ${weldMode === 'spot' ? 'bg-white text-red-600 shadow-md' : 'text-slate-500 hover:text-slate-700'}`}
              >
                Ponto
              </button>
            </div>
        ...
        <div className="flex-1 flex flex-col lg:flex-row overflow-hidden">
          ...
          <div className="flex-1 bg-slate-100 relative overflow-hidden">
             ...
             {/* Legend Overlay */}
             <div className="absolute bottom-6 left-6 bg-white/95 backdrop-blur p-4 rounded-2xl border border-slate-200 shadow-xl text-[11px] space-y-3 min-w-[180px]">
                <div className="font-bold text-slate-400 uppercase tracking-widest mb-1 border-b border-slate-100 pb-2">Legenda Técnica</div>
                <div className="flex items-center gap-3">
          ...
          {/* Sidebar Details */}
          <div className="w-full lg:w-[380px] bg-slate-50 border-l border-slate-200 flex flex-col shadow-inner">
             <div className="p-6 overflow-y-auto flex-1">
                <h3 className="text-xs font-black text-slate-400 uppercase tracking-[0.2em] mb-6">Elementos Definidos</h3>
                
                <div className="space-y-8">
                  <div>
                    <div className="flex items-center justify-between mb-3">
                      <h4 className="text-[11px] font-bold text-slate-500 uppercase tracking-widest">Soldas Definidas</h4>
                    ...
                    <div className="space-y-2 max-h-[200px] overflow-y-auto pr-2">
                      {parts.map(part => {
                        ...
                        return (
                          <div key={part.id} className="bg-white p-3 rounded-xl border border-slate-200 text-[11px] shadow-sm">
                            <div className="font-bold text-slate-800 mb-1">{part.name}</div>
                            <div className="text-slate-500">{welds.length} arestas marcadas</div>
                          </div>
                        );
                      })}
                      {allWelds.length === 0 && (
                        <div className="text-center py-10 border-2 border-dashed border-slate-200 rounded-2xl text-slate-400 text-[11px] italic">
                  ...
                  <div>
                    <div className="flex items-center justify-between mb-3">
                      <h4 className="text-[11px] font-bold text-slate-500 uppercase tracking-widest">Interseções</h4>
                    ...
                    <div className="space-y-2 max-h-[200px] overflow-y-auto pr-2">
                      {parts.map(part => {
                        ...
                        return (
                          <div key={part.id} className="bg-white p-3 rounded-xl border border-slate-200 text-[11px] shadow-sm">
                            <div className="font-bold text-slate-800 mb-1">{part.name}</div>
                            <div className="text-slate-500">{ints.length} pontos marcados</div>
                          </div>
                        );
                      })}
