    ...
    <div className="absolute top-4 left-4 z-20 pointer-events-auto">
      {!isPanelOpen ? (
        <button 
          ...
          className="p-2 bg-slate-900/80 backdrop-blur-md border border-slate-700 rounded-lg text-slate-400 hover:text-blue-400 transition-all shadow-xl"
        >
          <Activity size={16} />
        </button>
      ) : (
        <div className="bg-slate-900/95 backdrop-blur-md rounded border border-slate-700 shadow-2xl w-64 md:w-72 overflow-hidden flex flex-col">
          <div className="flex items-center justify-between gap-2 px-4 py-3 bg-slate-800 border-b border-slate-700">
            <div className="flex items-center gap-2">
              <Activity size={14} className="text-blue-400" />
              <h3 className="font-bold text-white text-[10px] uppercase tracking-[0.2em]">Data Analysis</h3>
            </div>
          ...
          <div className="p-4 space-y-4 text-[10px]">
            {tipoProduto === 'tesoura' ? (
              <div className="space-y-2 mb-2">
                <div className="flex justify-between items-center gap-2">
                  ...
                  <span className="font-mono text-slate-300 truncate text-right">{perfilData.nome}</span>
                </div>
                <div className="flex justify-between items-center gap-2">
                  <div className="flex items-center gap-1.5">
                  ...
                  <span className="font-mono text-slate-300 truncate text-right">{perfilVigaData?.nome || perfilData.nome}</span>
                </div>
                <div className="flex justify-between items-center gap-2">
                  <div className="flex items-center gap-1.5">
                  ...
                  <span className="font-mono text-slate-300 truncate text-right">{perfilDiagonalData?.nome || perfilVigaData?.nome || perfilData.nome}</span>
                </div>
              </div>
            ) : (
            ...
            <div className="space-y-1.5">
              <div className="flex justify-between">
                ...
                <span className="font-mono text-slate-300">{calculations.props.massaLinear.toFixed(2)} kg/m</span>
              </div>
              <div className="flex justify-between">
                <span className="text-slate-500 uppercase tracking-wider">Inércia:</span>
                <span className="font-mono text-slate-300">{(calculations.props.ix * 1e8).toFixed(2)} cm⁴</span>
              </div>
              <div className="flex justify-between">
                <span className="text-slate-500 uppercase tracking-wider">Vão (L):</span>
                <span className="font-mono text-slate-300">{calculations.vao.toFixed(2)} m</span>
              </div>
            </div>
            ...
            <div className={`mt-4 p-3 rounded border ${calculations.verificacao.aprovado ? 'bg-blue-600/10 border-blue-500/30' : 'bg-red-600/10 border-red-500/30'}`}>
              <div className="flex items-start gap-2">
                ...
                <div>
                  ...
                  <p className={`text-[9px] mt-1 font-mono ${calculations.verificacao.aprovado ? 'text-slate-400' : 'text-red-300'}`}>
