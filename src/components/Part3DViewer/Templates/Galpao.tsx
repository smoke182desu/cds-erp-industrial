...
export const Galpao: React.FC<GalpaoProps> = ({
  ...
  const [tesouraBOM, setTesouraBOM] = React.useState<any[]>([]);
    ...
    if (onBOMCalculated) {
      ...
      const baseBOM = [
        { 
          ...
          name: 'Peça A - Coluna Principal', 
          color: PART_COLORS.COLUNA, 
          quantity: 4, 
          unit: 'un',
          material: `${perfilColuna.nome} (${altura}mm)`,
          weight: 4 * (altura / 1000) * (perfilColuna.pesoPorMetro || 10),
          cost: 4 * (altura / 1000) * ((perfilColuna.pesoPorMetro || 10) * 15)
        },
        { 
          code: PART_CODES.TERCA, 
          name: 'Peça C - Terça de Cobertura', 
          color: PART_COLORS.TERCA, 
          quantity: (qtdTercasProp || 6) * 2, 
          unit: 'un',
          material: `${perfilTerca.nome} (${profundidade}mm)`,
          weight: ((qtdTercasProp || 6) * 2) * (profundidade / 1000) * (perfilTerca.pesoPorMetro || 10),
          cost: ((qtdTercasProp || 6) * 2) * (profundidade / 1000) * ((perfilTerca.pesoPorMetro || 10) * 15)
        },
        { 
          code: PART_CODES.SAPATA, 
          name: 'Peça F - Sapata de Fixação', 
          color: PART_COLORS.SAPATA, 
          quantity: 4, 
          unit: 'un',
          material: fixacao === 'sapata_parafuso' ? 'Chapa de Aço 12.7mm' : 'Barra de Ancoragem',
          weight: 4 * 5,
          cost: 4 * 85
        }
      ];
      ...
      if (materialCobertura === 'telha') {
        ...
        baseBOM.push({
          code: PART_CODES.TELHA,
          name: 'Peça E - Telha de Cobertura',
          color: PART_COLORS.TELHA,
          quantity: area,
          unit: 'm²',
          material: `${telha.nome}`,
          weight: area * 5,
      ...
      // Map Tesoura BOM to letter B
      const mappedTesouraBOM = tesouraBOM.map(item => ({
        ...item,
        name: `Peça B - Tesoura (${item.name})`,
        code: `B-${item.code}`
      }));

      onBOMCalculated([...baseBOM, ...mappedTesouraBOM]);
    }
  }, [largura, profundidade, altura, perfilData, perfilColunaDataProp, perfilTercaDataProp, telhaSelecionadaId, fixacao, materialCobertura, qtdTercasProp, tesouraBOM, onBOMCalculated]);
  ...
  const perfilTercaH = (perfilTercaData.tipoShape === 'redondo_oco' 
    ...
    : (perfilTercaData.tipoShape.includes('perfil_u') ? (perfilTercaData.largura || 75) : (perfilTercaData.altura || perfilTercaData.largura || 75))) / 1000;
  ...
  const trussSpacing = numTesouras > 1 ? d / (numTesouras - 1) : 0;
  ...
  const perfilBanzoH = (perfilData.tipoShape === 'redondo_oco' 
    ...
    : (perfilData.tipoShape.includes('perfil_u') ? (perfilData.abas || 40) : (perfilData.altura || perfilData.largura || 50))) / 1000;
  ...
  return (
    <group>
      ...
      <group position={exp(-0.3, 0, 0)}>
        ...
        {explodedFactor > 0.5 && <Label text="Colunas de Sustentação" position={[-w/2 - 0.5, h/2, 0]} />}
      </group>
      ...
      <group position={exp(0, 0.6, 0)}>
        ...
        />
        {explodedFactor > 0.5 && <Label text="Terças de Apoio" position={[0, ridgeHeight + 0.6, 0]} />}
      </group>
      ...
      <group position={exp(0, 0.3, 0)}>
            ...
            <group key={`truss-${i}`}>
              ...
              <group position={[0, trussOffset, 0]}>
                <Tesoura
                  ...
                  acabamentoMetal={acabamentoMetal}
                  explodedFactor={explodedFactor} // Pass explodedFactor to Tesoura
                  yOffset={h}
                  zOffset={z}
                  ...
                  onBOMCalculated={i === 0 ? setTesouraBOM : undefined}
                />
        ...
        {explodedFactor > 0.5 && <Label text="Tesouras de Cobertura" position={[0, ridgeHeight + 0.3, 0]} />}
      </group>

      {/* Cobertura (Telhas) */}
      {materialCobertura === 'telha' && (
        <group position={exp(0, 1.0, 0)}>
          {(() => {
            const telha = telhasDB.find(t => t.id === telhaSelecionadaId) || telhasDB[0];
          ...
          {explodedFactor > 0.5 && <Label text="Cobertura Metálica" position={[0, ridgeHeight + 1.0, 0]} />}
        </group>
      )}
