...
import { Center, Html, Text, Billboard } from '@react-three/drei';
...
export const SheetMetal3D: React.FC<SheetMetal3DProps> = ({
  ...
  const w = Math.max(0.001, (Number(largura) || 100) / 1000);
  const h = Math.max(0.001, (Number(altura) || 100) / 1000);
  const d = Math.max(0.001, (Number(profundidade) || 1000) / 1000);
  const t = Math.max(0.001, (Number(espessura) || 1.2) / 1000); // Mínimo 1mm para evitar quebra WebGL
  const ae = Math.max(0.001, (Number(abaExtra) || 15) / 1000);

  React.useEffect(() => {
    if (onBOMCalculated) {
      let devW = w;
      if (tipo === 'chapa_dobrada_l') devW = w + h;
      else if (tipo === 'chapa_dobrada_u' || tipo === 'perfil_u_enrijecido') devW = w + (2 * h);
      else if (tipo === 'bandeja_metalica') devW = w + (2 * h) + (2 * ae);

      const area = devW * d;
      const weight = area * (t * 1000) * 7.85; // Peso aproximado aço carbono (7.85 kg/m² por mm)

      const bom = [
        {
          code: 'SM-01',
          name: `Chapa ${tipo.replace(/_/g, ' ').toUpperCase()}`,
          quantity: area,
          unit: 'm²',
          material: `Chapa Aço ${t * 1000}mm`,
          weight: weight,
          cost: weight * 12.0 // Custo aproximado por kg
        }
      ];
      onBOMCalculated(bom);
    }
  }, [onBOMCalculated, tipo, w, h, d, t, ae]);
      ...
      <group rotation={[0, Math.PI / 4, 0]}>
        ...
        {mostrarCotas && (
          !planificada ? (
            <>
              {/* Cota LARGURA (Base) */}
              <Label position={[w / 2, -0.05, d / 2]} text={`L: ${Number(largura)} mm`} />
              
              {/* Cota ALTURA (Abas) - Só se não for chapa cortada */}
              {tipo !== 'chapa_cortada' && (
                <Label position={[-0.05, h / 2, d / 2]} text={`H: ${Number(altura)} mm`} />
              )}
              
              {/* Cota COMPRIMENTO (Profundidade) */}
              <Label position={[w + 0.05, h / 2, d / 2]} text={`C: ${Number(profundidade)} mm`} />
            </>
          ) : (
            <>
