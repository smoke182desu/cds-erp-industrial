...
function LongWaitMessage() {
  ...
  return (
    <motion.span 
      initial={{ opacity: 0 }}
      animate={{ opacity: 1 }}
      className="text-[10px] text-slate-600 italic flex items-center gap-1"
    >
...
export function AIChat({ project, onAIProjectUpdate, onUndo, onClose }: AIChatProps) {
  ...
  return (
    <div className="flex flex-col h-full bg-white border-l border-slate-300 w-96 shadow-xl z-20">
      <div className="p-4 border-b border-slate-300 bg-slate-50 flex justify-between items-center">
        <div className="flex items-center gap-3">
          {onClose && (
            <button 
              onClick={onClose}
              className="p-1.5 hover:bg-slate-100 rounded text-slate-600 lg:hidden"
            >
              <XCircle size={18} />
            </button>
          )}
        ...
        <button onClick={onUndo} className="p-1.5 hover:bg-slate-100 rounded text-slate-600 transition-colors" title="Desfazer">
          <Undo2 size={16} />
        </button>
      </div>

      <div className="flex-1 overflow-y-auto p-4 space-y-4 bg-white/50 scrollbar-thin scrollbar-thumb-slate-700 scrollbar-track-transparent">
        {messages.map((msg) => (
          <motion.div 
          ...
          >
            ...
            <div className={`max-w-[85%] p-3.5 rounded-2xl text-sm leading-relaxed shadow-sm ${
              msg.role === 'user' 
                ...
                : (msg.content.includes("problema técnico") ? 'bg-red-50 border border-red-200 text-red-700 rounded-tl-none' : 'bg-slate-100 text-slate-800 border border-slate-200 rounded-tl-none')
            }`}>
              <p className="whitespace-pre-line">{msg.content}</p>
              {msg.content.includes("problema técnico") && (
                  <button 
                    onClick={handleRetry}
                    className="mt-2 text-xs flex items-center gap-1 text-red-300 hover:text-slate-900 transition-colors underline"
                  >
        ...
        {isAnalyzing && (
          ...
          >
            ...
            <div className="bg-slate-100 text-slate-800 border border-slate-300 rounded-2xl rounded-tl-none p-4 flex flex-col gap-3 relative group min-w-[240px]">
      ...
      <div className="p-4 bg-slate-50 border-t border-slate-200">
        ...
        <div className="relative flex items-center gap-2">
          ...
          <button 
            ...
            className="p-2.5 bg-slate-100 hover:bg-slate-200 text-slate-600 rounded-lg transition-colors border border-slate-300"
            title="Anexar Imagem"
          >
            <ImageIcon size={18} />
          </button>
          ...
          <div className="relative flex-1">
            <input
              ...
              className="w-full bg-white border border-slate-300 rounded-lg pl-4 pr-12 py-3 text-sm text-slate-900 focus:border-emerald-500 focus:ring-1 focus:ring-emerald-500 outline-none shadow-inner disabled:opacity-50"
            />
            <button 
              onClick={handleSend}
              disabled={isAnalyzing}
              className="absolute right-2 top-1/2 -translate-y-1/2 p-1.5 bg-emerald-600 hover:bg-emerald-500 text-white rounded-md transition-colors disabled:bg-slate-200"
            >
