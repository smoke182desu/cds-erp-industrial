...
export function CuttingPlan({ project }: CuttingPlanProps) {
  ...
  return (
    <div className="space-y-8 p-8 max-w-5xl mx-auto font-sans">
      ...
      {linearBars.length > 0 && (
        <div className="space-y-6">
          ...
          {linearBars.map((bar, index) => (
            <div key={bar.id} className="bg-white rounded-xl shadow-sm border border-slate-200 overflow-hidden print:border-2 print:border-black break-inside-avoid">
              <div className="bg-slate-50 border-b border-slate-200 p-4 flex justify-between items-center">
                <h3 className="font-bold text-slate-800 flex items-center gap-2">
                  <span className="bg-orange-600 text-white px-2 py-0.5 rounded text-xs">BARRA {index + 1}</span>
                  {bar.material} - 6000mm
                </h3>
              ...
              <div className="p-6">
                ...
                <div className="relative w-full h-16 bg-slate-200 border-2 border-slate-400 rounded flex items-center overflow-hidden mb-4">
                   ...
                   <div className="flex-1 h-full bg-slate-100 flex items-center justify-center">
                      <span className="text-[10px] text-slate-400 italic">Sobra ({bar.leftover}mm)</span>
                   </div>
                </div>

                {/* Cut List */}
                <div className="bg-slate-900 text-slate-200 p-4 rounded-lg text-sm font-mono print:bg-white print:text-black print:border print:border-black mb-4">
                   <h4 className="text-orange-400 font-bold uppercase mb-3 border-b border-slate-700 pb-2 print:text-black print:border-black">Sequência de Cortes (Policorte)</h4>
                   <ol className="list-decimal list-inside space-y-1 text-xs">
                      {bar.cuts.map((cut, cIdx) => (
                        <li key={cIdx} className="print:text-black">
                           <span className="text-slate-400 print:text-black">Posição {cut.pos}mm:</span> {cut.description}
                        </li>
                      ))}
                   </ol>
                </div>
                ...
                <div className="bg-white border border-slate-200 rounded-lg p-4 print:border-black">
                  <h4 className="text-slate-800 font-bold text-sm mb-3 uppercase border-b pb-2 flex items-center gap-2 print:text-black print:border-black">
                    <span className="w-2 h-2 bg-orange-500 rounded-full"></span>
                    Detalhes das Peças e Conexões
                  </h4>
                  <div className="grid md:grid-cols-2 gap-4">
                    {bar.parts.map((part, pIdx) => (
                      <div key={pIdx} className="text-xs text-slate-600 bg-slate-50 p-3 rounded border border-slate-100 print:bg-white print:border-black">
                        <div className="flex justify-between font-bold text-slate-800 mb-1 print:text-black">
                          <span className="flex items-center gap-1">
                            <span className="w-4 h-4 bg-orange-100 text-orange-700 rounded flex items-center justify-center text-[9px] border border-orange-200 print:border-black print:text-black print:bg-transparent">{pIdx + 1}</span>
                            {part.name}
                          </span>
                          <span className="text-slate-500 font-mono print:text-black">{part.length}mm</span>
                        </div>
                        
                        {part.originalComponent?.welds && part.originalComponent.welds.length > 0 ? (
                          <div className="mt-2 pl-2 border-l-2 border-orange-200 print:border-black">
                            <span className="block text-[10px] text-orange-600 font-bold uppercase mb-0.5 print:text-black">Conecta com:</span>
                            <ul className="space-y-1">
                              {part.originalComponent.welds.map((weld, wIdx) => (
                                <li key={wIdx} className="text-[10px] leading-tight print:text-black">
                                  {weld.targetId ? (
                          ...
                          <div className="mt-1 pl-2 text-[10px] text-slate-400 italic print:text-black">Sem conexões de solda especificadas.</div>
                        )}
                      </div>
                    ))}
                  </div>
      ...
      {Object.entries(sheetsByMaterial).map(([material, materialSheets]) => (
        <div key={material} className="space-y-6 mt-8">
           <h2 className="font-bold text-slate-800 flex items-center gap-2 text-xl border-b pb-2">
            ...
            Plano de Corte de Chapas ({material}) - Guilhotina/Laser
          </h2>
          {materialSheets.map((sheet, index) => (
            <div key={index} className="bg-white rounded-xl shadow-sm border border-slate-200 overflow-hidden print:border-2 print:border-black break-inside-avoid">
              <div className="bg-slate-50 border-b border-slate-200 p-4 flex justify-between items-center">
              ...
              <div className="p-6">
                <div className="relative w-full aspect-[2.5/1] bg-slate-100 border-2 border-slate-800 mb-6 overflow-hidden">
                  {/* Render Leftovers */}
                  ...
                  {sheet.components.map((comp, cIdx) => {
                    ...
                    return (
                      ...
                      >
                        <div className="font-black bg-slate-800 text-white px-2 py-0.5 rounded text-xs mb-1">
                          {comp.id}
                        </div>
                  ...
                  {sheet.cuts?.map((cut, cIdx) => {
                    ...
                    if (cut.axis === 'x') {
                      return (
                        ...
                        >
                          <div className={`absolute ${bgClass} text-white rounded-full w-5 h-5 flex items-center justify-center text-[10px] font-bold shadow-sm ring-2 ring-white`}>
                            {cut.id}
                          </div>
                        </div>
                      );
                ...
                <div className="bg-slate-900 text-slate-200 p-4 rounded-lg text-sm font-mono print:bg-white print:text-black print:border print:border-black">
                  <h4 className="text-blue-400 font-bold uppercase mb-3 border-b border-slate-700 pb-2 print:text-black print:border-black">Guia de Máquina - Chapa {String.fromCharCode(65 + index)}</h4>
                  
                  <div className="mb-4">
                    <h5 className="text-slate-400 text-xs mb-2 uppercase tracking-wider print:text-black">Sequência de Cortes</h5>
                    <ol className="list-none space-y-2 text-xs">
                      {sheet.cuts?.length > 0 ? sheet.cuts.map((cut, cIdx) => (
                        <li key={cIdx} className="flex items-start gap-2">
                          <span className={`inline-flex items-center justify-center w-4 h-4 rounded-full text-[9px] font-bold text-white ${cutBgColors[cIdx % cutBgColors.length]} mt-0.5 shrink-0 print:border print:border-black print:text-black print:bg-transparent`}>
                            {cut.id}
                          </span>
                          <span>
                            <span className="font-bold text-white print:text-black">Corte {cut.id}:</span> {cut.description}
                          </span>
                        </li>
                      )) : (
                         <li className="text-slate-500 italic print:text-black">Nenhum corte registrado.</li>
                      )}
                    </ol>
                  </div>

                  <div>
                    <h5 className="text-slate-400 text-xs mb-2 uppercase tracking-wider print:text-black">Peças Extraídas</h5>
                    <ul className="list-disc list-inside space-y-1 text-xs mb-4">
                      {sheet.components.map((comp, cIdx) => (
                        <li key={cIdx}>
                          <span className="font-bold text-white print:text-black">{comp.name}</span> ({comp.width} x {comp.height} mm) - ID: {comp.id} - Peso: {calculatePartWeight(comp)} kg
                        </li>
                      ))}
                    </ul>
                  </div>
                  ...
                  {sheet.leftovers && sheet.leftovers.length > 0 && (
                    <div>
                      <h5 className="text-slate-400 text-xs mb-2 uppercase tracking-wider print:text-black">Retalhos / Sobras</h5>
                      <ul className="list-disc list-inside space-y-1 text-xs">
                        {sheet.leftovers.map((leftover, lIdx) => (
                          <li key={lIdx} className="text-slate-400 print:text-black">
      ...
      <div className="bg-white rounded-xl shadow-sm border border-slate-200 overflow-hidden print:border-2 print:border-black break-inside-avoid mt-8">
        <div className="bg-slate-50 border-b border-slate-200 p-4 flex justify-between items-center print:bg-white print:border-black">
          <h2 className="font-bold text-slate-800 flex items-center gap-2 text-xl">
            <Wrench size={24} className="text-indigo-600 print:text-black" />
        ...
        <div className="p-6">
          <div className="space-y-6">
            {assemblySteps.map((step, idx) => (
              <div key={idx} className="border-b border-slate-100 pb-6 last:border-0 last:pb-0 print:border-black">
                <div className="flex justify-between items-start mb-2">
                  <h3 className="font-bold text-slate-800 text-lg">{step.title}</h3>
                  {getActionBadge(step.action)}
                </div>
                <p className="text-slate-600 mb-3 text-sm print:text-black">{step.description}</p>
                
                {step.parts.length > 0 && (
                  <div className="bg-slate-50 rounded p-3 border border-slate-200 print:bg-white print:border-black">
                    <span className="text-xs font-bold text-slate-500 uppercase tracking-wider mb-2 block print:text-black">Peças Envolvidas:</span>
                    <ul className="list-disc list-inside text-sm text-slate-700 font-medium print:text-black">
